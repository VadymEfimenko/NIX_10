...
public class PLPController {
    ...
    private String allReleases(Model model, WebRequest webRequest) {
        PageData<ReleasePLPDto> releases = plpFacade.search(webRequest);
        model.addAttribute("pageData", releases);
    ...
    public ModelAndView findAllRedirect(WebRequest webRequest, ModelMap modelMap) {
        ...
        return new ModelAndView("redirect:/releases", modelMap);
    }
    ...
    public String redirectToNewReleasePage(@PathVariable Long id, Model model) {
        model.addAttribute("releases", releaseFacade.findById(id));
